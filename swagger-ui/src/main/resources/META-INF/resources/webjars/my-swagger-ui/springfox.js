$(function() {
  ...
  var springfox = {
    "getBaseUrl": function() {
      if (baseUrl === undefined) {
        var urlMatches = /(.*)\/my-swagger-ui.html.*/.exec(window.location.href);
  ...
  window.oAuthRedirectUrl = springfox.getBaseUrl() + '/webjars/my-swagger-ui/o2c.html';
  ...
  function initializeBaseUrl() {
    ...
    $.getJSON(relativeLocation + "/swagger-ext/module-resources.json", function(data) {
      ...
      $.each(data, function(i, resource) {
        var option = $('<option></option>')
            ...
            .attr("data-module-id", resource.moduleId || '')
            .text(resource.name + " (" + resource.location + ")");
        if(hashs.length===3 && hashs[1]==resource.moduleId) {
  ...
  function onSuccess(data) {
    window.swaggerUi = new SwaggerUi({
      ...
      onComplete: function(swaggerApi, swaggerUi) {
        ...
        if(hashs.length===3) {
    	  var elem = $('#' + Docs.escapeResourceName(hashs[1] + '_' + hashs[2] + '_content')).parents('.endpoints').show();
        }
      },
